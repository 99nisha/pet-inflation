...
forecast_prophet_2016_2024 <- forecast_prophet_2016_2024 %>%
...
## Forecast Analysis with Regressors on 2025 from (2016-2024) historic data
...
autoplot(forecast_arimax_2016_2024) +
  ggtitle("ARIMAX Forecast of CPI for Veterinary Services (2025)") +
  xlab("Year") +
  ylab("CPI Index") +
  theme_minimal()
...
# --- Compare ARIMA vs ARIMAX Performance ---
...
No VIF > 5, so multicollinearity is not severe. ✅
Air_Frost and Mean_Temp have the highest VIFs (~4.6–4.9), indicating some correlation with other variables, but it’s not alarming.
GDP and Rainfall are almost independent of the others — coefficients should be stable.


```{r}

# Only significant regressor
xreg_final <- as.matrix(wide_data_2016_2024 %>% select(Rainfall))

fit_arimax_final <- auto.arima(ts_cpi_2016_2024,
                               xreg = xreg_final,
                               seasonal = FALSE)

summary(fit_arimax_final)

AIC(fit_arimax_final)
BIC(fit_arimax_final)
