...
forecast_ets_2016_2024 <- forecast(fit_ets_2016_2024, h=24)  # Forecast 24 months (2025 2026)

fit_arima_2016_2024 <- auto.arima(ts_data_2016_2024, seasonal=FALSE)
forecast_arima_2016_2024 <- forecast(fit_arima_2016_2024, h=24)

fit_sarima_2016_2024 <- auto.arima(ts_data_2016_2024, seasonal=TRUE, stepwise=FALSE, approximation=FALSE)
forecast_sarima_2016_2024 <- forecast(fit_sarima_2016_2024, h=24)
...
future_2016_2024 <- make_future_dataframe(m, periods = 26, freq = 'month')
forecast_prophet_2016_2024 <- predict(m, future_2016_2024)

forecast_prophet_2025 <- forecast_prophet_2016_2024 %>%
  dplyr::filter(ds >= as.Date("2025-01-01") & ds <= as.Date("2026-12-31"))
...
xreg_train_2016_2024 <- as.matrix(
  wide_data_2016_2024 %>%
    select(Air_Frost, GDP, Mean_Temp, Rainfall, Wages, Pet_Population)
)

# Interpolate missing values in regressors
xreg_train_filled <- apply(xreg_train_2016_2024, 2, function(col) na.approx(col, rule = 2))
xreg_train_filled <- as.matrix(xreg_train_filled)
colnames(xreg_train_filled) <- c("Air_Frost", "GDP", "Mean_Temp", "Rainfall", "Pet_Population", "Wages")

# Standardize regressors
xreg_mean <- colMeans(xreg_train_filled, na.rm = TRUE)
xreg_sd <- apply(xreg_train_filled, 2, sd, na.rm = TRUE)

xreg_train_scaled <- scale(xreg_train_filled, center = xreg_mean, scale = xreg_sd)

# Prepare future xreg using last interpolated row
last_row <- tail(xreg_train_filled, 1)
future_xreg_2016_2024 <- matrix(
  rep(as.numeric(last_row), 24),
  ncol = ncol(xreg_train_filled),
  byrow = TRUE
)
colnames(future_xreg_2016_2024) <- colnames(xreg_train_filled)


# Standardize future regressors with same mean & sd
future_xreg_scaled <- scale(future_xreg_2016_2024, center = xreg_mean, scale = xreg_sd)

# --- Fit ARIMAX ---
fit_arimax_2016_2024 <- auto.arima(ts_cpi_2016_2024,
                                   xreg = xreg_train_filled,
                                   seasonal = FALSE)

forecast_arimax_2016_2024 <- forecast(fit_arimax_2016_2024,
                                      xreg = future_xreg_2016_2024,
                                      h = 24)

# --- Fit SARIMAX ---
fit_sarimax_2016_2024 <- auto.arima(ts_cpi_2016_2024,
                                    xreg = xreg_train_filled,
                                    seasonal = TRUE,
                                    stepwise = FALSE,
                                    approximation = FALSE)

forecast_sarimax_2016_2024 <- forecast(fit_sarimax_2016_2024,
                                       xreg = future_xreg_2016_2024,
                                       h = 24)
...
arimax_df <- data.frame(
  Date = seq.Date(from = as.Date("2025-01-01"), by = "month", length.out = 24),
